...
namespace CL
{
    void AABB::expand(const clm::vec3 &point)
    {
    ...
    bool AABB::intersectsRay(const clm::vec3 &origin, const clm::vec3 &invDir) const
    {
    ...
    void BVH::buildRecursive(uint32_t currNodeIndex)
    {
        AABB centroidVol;
        ...
        for (const Triangle &triangle : nodes[currNodeIndex].triangles)
        {
            ...
            centroidVol.expand(triangle.centroid());
        }

        if (nodes[currNodeIndex].triangles.size() <= 2)
            return;

        const clm::vec3 centroidVolSize(fabsf(centroidVol.max.x - centroidVol.min.x),
                                        fabsf(centroidVol.max.y - centroidVol.min.y),
                                        fabsf(centroidVol.max.z - centroidVol.min.z));
        ...
        if (centroidVolSize.x > centroidVolSize.y && centroidVolSize.x > centroidVolSize.z)
        {
            const float mid = midPoint(centroidVol.min.x, centroidVol.max.x);
        ...
        else if (centroidVolSize.y > centroidVolSize.x && centroidVolSize.y > centroidVolSize.z)
        {
            const float mid = midPoint(centroidVol.min.y, centroidVol.max.y);
        ...
        {
            const float mid = midPoint(centroidVol.min.z, centroidVol.max.z);
    ...
    BVH::BVH(const std::vector<Model> &models)
    {
    ...
    std::vector<uint32_t> BVH::getHitLeaves(const clm::vec3 &rayOrigin, const clm::vec3 &rayDir) const
    {
